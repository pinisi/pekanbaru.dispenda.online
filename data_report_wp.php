...
. $where . " group by wpName, device.deviceid, DATE_FORMAT(tgltransaksi,'%M-%Y')";
...
$SQL = "SELECT device.deviceid, wpName, DATE_FORMAT(tgltransaksi,'%M-%Y') as 'bulan', sum(round( (select nilai_pajak from kategori where device.kategoriid = id) * struk.jumlah, 0)) as pajak,address, (select kategori_name from kategori where id = device.kategoriid) as kategori "
. " FROM struk,device"
. " WHERE struk.deviceid = device.deviceid"
. $where . " group by wpName, device.deviceid, DATE_FORMAT(tgltransaksi,'%M-%Y')"
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
    ...
    echo "<cell>". $row['deviceid']."|".$row['bulan']."|" . $row['pajak'] . "|" . $row['wpName']. "|" . $row['address'] . "|" . $row['kategori'] . "</cell>";
    echo "</row>";
